...
| 英文本地化 (English localization) | `SuixinJi/Localizable.xcstrings` String Catalog (source language **zh-Hans**, 176 English translations of the app's user-facing strings). SwiftUI `Text`/`Label`/`Button`/`navigationTitle`/alerts auto-localize; anything untranslated **falls back to Chinese** (safe, partial coverage). `en` added to `knownRegions`; UI tests pin the app to zh-Hans via `-AppleLanguages`. **Scope:** the main app UI is translated; interpolated/date-format strings, and the widget + share-extension bundles, still show Chinese (a natural follow-up). |
