...
| 锁屏小组件 (Lock Screen) | Same `TodayWidget`, extended `supportedFamilies` with `.accessoryInline` / `.accessoryCircular` / `.accessoryRectangular` — glanceable streak / today status on the Lock Screen, reusing the same `WidgetSnapshot`. Accessory families render tinted (`AccessoryWidgetBackground`, `widgetAccentable`). |
| 分享扩展 (Share Extension) (share text / images from any app into 随心记) | New `SuixinJiShareExtension` target (`ShareViewController` : `SLComposeServiceViewController`). The extension is a **dumb writer**: it drops the text + image files into the App Group `ShareInbox` (Shared/). The app drains the inbox into new entries on launch/active via `ShareImporter` → `DiaryService.save` (no cross-process SwiftData). Bundle id `app.suixinji.mj.share`, App Group `group.app.suixinji.mj`. See below. |
| 数据加密 · 加密备份 (password-encrypted backup) | `BackupCrypto` (CryptoKit AES-256-GCM + PBKDF2-HMAC-SHA256 key derivation, envelope = magic ‖ salt ‖ combined); pure & tested. `BackupService.writeBackupFile(password:)` → `.suixinji`, `importData(password:)` auto-detects & decrypts. Settings 导出 offers 直接/加密（设密码）; encrypted import prompts for the password (wrong password → clear error). On-device files already use iOS `CompleteUntilFirstUserAuthentication` protection; this closes the gap for the *exported* file. |
| Live Activity · 写作计时 (writing timer on Lock Screen + Dynamic Island) | `WritingActivityAttributes` (Shared/, ActivityKit) + `WritingLiveActivity` (`ActivityConfiguration` in the widget extension: Lock Screen banner + Dynamic Island compact/minimal/expanded) + `WritingActivityController` (app: start/update/end, best-effort). Editor 专注 button toggles it; the elapsed timer auto-runs via `Text(timerInterval:)` (no pushes), char count is throttled. `NSSupportsLiveActivities=YES` on the app. Ends on editor dismissal. |
| 英文本地化 (English localization) | `SuixinJi/Localizable.xcstrings` String Catalog (source language **zh-Hans**, ~190 English translations incl. interpolated `%lld`/`%@` keys). SwiftUI `Text`/`Label`/`Button`/`navigationTitle`/alerts auto-localize; `Text(stringVar)` spots (font-size / iCloud status / prompt) wrapped in `LocalizedStringKey`; anything untranslated **falls back to Chinese** (safe). `en` in `knownRegions`; UI tests pin zh-Hans via `-AppleLanguages`. **Dates** are locale-aware (`DiaryDateFormat` branches: exact zh patterns preserved, `setLocalizedDateFormatFromTemplate` for other languages). The **widget** and **share-extension** bundles have their own `Localizable.xcstrings`. |
...
| 标签管理 + 组合筛选 (rename/merge/delete tags across all entries; filter the timeline by any-of moods / any-of tags / require photo / require audio) | `TagManager` (pure list transforms + context mutators; rename-onto-existing merges) + `TagManagementView` (Settings → 标签管理); `DiaryFilter` (pure, AND across facets / OR within) + `FilterSheet` + Home filter button/active-bar, composed with `DiarySearch`. |
| 富文本 Markdown (render headings / lists / quote / divider + inline **bold** *italic* `code` links in the detail view; clean stripped preview on cards) | `MarkdownParser` (pure, line-based blocks + `plainPreview`) + `MarkdownText` (blocks via SwiftUI, inline via `AttributedString(markdown:)`). Storage stays **plain text** — rendered at display time only; `DetailView` renders rich, `DiaryCardView` shows the stripped preview. |
...
(`group.app.suixinji.mj`). `Shared/` (snapshot, `SharedStore`, `QuickAddIntent`)
...
1. Target *SuixinJi* → *Signing & Capabilities* → **+ Capability → iCloud** →
   check **CloudKit**, container `iCloud.app.suixinji.mj`. The entitlements are
